...
public class CompressionEncryption extends WorkflowStep {
	...
	public boolean run(WorkflowContext context) {
		...
		try {
			...
			List<String> folders = getDirectories(output);
			...
			if (pgsPanel != null) {
				...
				// TODO: check this function and the extension. Is it the same as
				// HdfsUtil.getFile(outputScore);
				String[] scoreList = FileUtil.getFiles(outputScores, ".txt");

				String[] chunksScores = new String[scoreList.length / 2];
				String[] chunksReports = new String[scoreList.length / 2];

				int chunksScoresCount = 0;
				int chunksReportsCount = 0;

				for (String score : scoreList) {
					String localPath = score;
